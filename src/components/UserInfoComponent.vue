...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    ...
    r.then((response: AxiosResponse<any>) => {
      ...
      //alert(JSON.stringify(form)) //for debug
...
<template>
  <div style="display: flex; flex-direction: column">
    ...
    <div v-else style="display: flex; flex-direction: row-reverse; align-items: flex-end">
      <el-button type="primary" @click="edit = true; setForm()" icon="Edit">编辑</el-button>
    </div>
    ...
    <el-form ref="ruleFormRef" :model="form" :rules="rules" label-width="30%" class="demo-ruleForm" label-position="right"
      ...
      <el-form-item label="用户类型：" prop="user_type">
        <!-- <el-input v-model="form.user_type" style="width: 25vh" :disabled="true" /> -->
        <el-text>{{ form.user_type }}</el-text>
      </el-form-item>
      ...
      <el-form-item label="积分：" prop="mileage_points">
        ...
        <el-text>{{ form.mileage_points }}</el-text>
      </el-form-item>
    </el-form>
  </div>
</template>
